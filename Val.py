...
func = ODEFunc().cuda()
func.load_state_dict(torch.load("non_ode_ReLU_model_3.pth", weights_only=True))
...
with torch.no_grad():
    for batch in tqdm(val_loader):
        ...
        #print(t[-1]-t0)
        y0 = torch.cat([u0,torch.tensor([[0,0,0]]),x0[:,3:],(t[-1]-t0).reshape([1,-1])],1)
        y = torch.cat([u,x[:,:3]-x0[:,:3],x[:,3:]],1)
        pred = func(y0.cuda())
        loss = criterion(torch.squeeze(pred.cpu().detach()),y[-1,8:])
        #print(torch.squeeze(pred.cpu().detach()).shape,y.shape)
        # if torch.isnan(loss):
        #     break
        #print((torch.squeeze(pred.cpu().detach())[:3] - y[-1,8:11]).reshape([-1,3]))
        L2 = torch.sqrt(torch.sum(torch.square(torch.squeeze(pred.cpu().detach())[:3] - y[-1,8:11]).reshape([-1,3]),dim=1))[-1]
