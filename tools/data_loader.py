...
class PMNNDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        u = torch.tensor(seq_data[::self.time_gap,  :7],dtype=torch.float32)    # all motor inputs
        x = torch.tensor(seq_data[::self.time_gap,  7:-1],dtype=torch.float32)  # orientation stuff
...
def dynamic_collate_fn(batch, window_size):
    batch_t0 = torch.stack([t[0] for u,x,t in batch]).reshape((-1, 1))
    ...
    if window_size < 0:
        batch_t = torch.stack([t[1:] for u,x,t in batch])
        batch_u = torch.stack([u[1:] for u,x,t in batch])
        batch_x = torch.stack([x[1:] for u,x,t in batch])
    ...
    for u,x,t in batch:
        max_t0 = len(t) - window_size
        num_of_windows = len(t) // window_size - 1
        seq_t0 = np.random.choice(range(max_t0),num_of_windows,replace=False)
        batch_t0 = torch.cat([batch_t0, torch.stack([t[st0] for st0 in seq_t0]).reshape((-1, 1))])
